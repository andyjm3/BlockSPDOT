% data to mnist has been removed due to size issue. Please run the code for fashionmnist and letters dataset instead!
...
% subsample
filename = 'mnist_pca75'; %fashionmnist_pca75,mnist_pca75,letter_pca100
m = 250;
n = 100;
d = 5;
k = d;
...
spdOT_cvx_result = nan(length(frac_vec),length(rand_seeds)); %for spdOT cvx proposedOT results
...
data_train = data_train/1; %division by 255 for fashionMNIST, 1 for mnist, 15 for letter
data_test = data_test/1; %division by 255 for fashionMNIST, 1 for mnist, 15 for letter
